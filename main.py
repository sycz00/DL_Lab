...
def retrieval():
	trained_embeddings = utils.open_pickle(cfg.DIR.SHAPENET_METRIC_EMBEDDINGS_TRAIN)
	print(trained_embeddings.keys())
	print(trained_embeddings['caption_embedding_tuples'][0])
	print(trained_embeddings['class_labels'])
	input()
...
def val(val_queue,val_process, text_encoder, shape_encoder,opts):
    ...
    print("Mean precision for {0} neighbors is : {1}".format(n_neighbors,np.mean(precision)))
